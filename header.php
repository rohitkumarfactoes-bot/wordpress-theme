 ...
 * Row 1: Logo LEFT | Search + DarkMode RIGHT
 * Row 2: Primary Navigation strip (full width, below brand bar)
 *
...
<div class="progress-bar" id="reading-progress" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
		...
		<div class="header-top__inner">
			...
			<a class="site-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home">
				...
				<span class="logo-text">
					<?php
					$n = get_bloginfo('name');
					// Split at last Capital group: GizmodoTech → Gizmodo + Tech
			...
			<div class="header-actions">
				<button class="hdr-btn search-toggle" type="button"
				        aria-label="<?php esc_attr_e('Search','gizmodotech-pro'); ?>"
				...
				<button class="hdr-btn dark-mode-toggle" id="dark-mode-toggle" type="button"
				        aria-label="<?php esc_attr_e('Toggle dark mode','gizmodotech-pro'); ?>" aria-pressed="false">
...
<nav class="mobile-nav" id="mobile-nav" aria-hidden="true"
     aria-label="<?php esc_attr_e('Mobile Navigation','gizmodotech-pro'); ?>">
	<div class="mobile-nav__head">
		<span class="mobile-nav__title"><?php esc_html_e('Menu','gizmodotech-pro'); ?></span>
		<button class="mobile-nav__close" type="button"
		        aria-label="<?php esc_attr_e('Close menu','gizmodotech-pro'); ?>">
...
<!-- Search Overlay -->
<div class="search-overlay" id="search-overlay" role="dialog" aria-modal="true" aria-hidden="true">
	<div class="search-overlay__box">
		<form role="search" method="get" class="search-overlay__form" action="<?php echo esc_url(home_url('/')); ?>">
			<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;opacity:.4" aria-hidden="true"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
			<label class="screen-reader-text" for="s-overlay"><?php esc_html_e('Search','gizmodotech-pro'); ?></label>
			<input type="search" id="s-overlay" name="s" class="search-overlay__input"
			       placeholder="<?php esc_attr_e('Search phones, reviews, news…','gizmodotech-pro'); ?>"
			       value="<?php echo esc_attr(get_search_query()); ?>" autocomplete="off">
			<button type="button" class="search-overlay__close"
			        aria-label="<?php esc_attr_e('Close','gizmodotech-pro'); ?>">
				<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
			</button>
		</form>
		<p style="padding:.5rem 1.5rem 1rem;font-size:.8rem;color:var(--text-muted);"><?php esc_html_e('Press Enter to search · Esc to close','gizmodotech-pro'); ?></p>
	</div>
</div>
